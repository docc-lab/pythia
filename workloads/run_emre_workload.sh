...
~/pythia/workloads/create_delete_no_OSP_workload.sh 1 -v
rm -r $HOME/trace-results

pids=()
for i in `seq $1`
do
	~/pythia/workloads/create_delete_vm.sh 1 -v &
        pids+=($!)
	~/pythia/workloads/create_delete_ip.sh 1 -v &
        pids+=($!)
	~/pythia/workloads/create_delete_volume.sh 1 -v &
        pids+=($!)
done
